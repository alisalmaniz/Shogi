...
public class K extends Man{
    ...
    public static String[] move( String p){
        ...
        if(board[i][j].charAt(2)=='1'){
            ...
            else if( !change[3].contains("e") &&  j+1<9 && (board[i][j+1].equals("----") || board[i][j+1].charAt(1)=='2' || board[i][j+1].charAt(2)=='2') ){
                change[0]=String.valueOf(10*i+(j+1));
                change[1]=board[i][j+1];
            ...
            else if( !change[3].contains("g") && (i-1>=0 && j-1>=0) && (board[i-1][j-1].equals("----") || board[i-1][j-1].charAt(1)=='2' || board[i-1][j-1].charAt(2)=='2') ){
                change[0]=String.valueOf(10*(i-1)+(j-1));
                change[1]=board[i-1][j-1];
                change[3]=change[3].concat("g");
            }
            else if( !change[3].contains("h") && (i-1>=0 && j+1>=0) && (board[i-1][j+1].equals("----") || board[i-1][j+1].charAt(1)=='2' || board[i-1][j+1].charAt(2)=='2') ){
                change[0]=String.valueOf(10*(i-1)+(j+1));
                change[1]=board[i-1][j+1];
            ...
            else if( !change[3].contains("g") && (i+1<9 && j+1<9) && (board[i+1][j+1].equals("----") || board[i+1][j+1].charAt(1)=='1' || board[i+1][j+1].charAt(2)=='1') ){
                change[0]=String.valueOf(10*(i+1)+(j+1));
                change[1]=board[i+1][j+1];
                change[3]=change[3].concat("g");
            }
            else if( !change[3].contains("h") && (i+1<9 && j-1>=0) && (board[i+1][j-1].equals("----") || board[i+1][j-1].charAt(1)=='1' || board[i+1][j-1].charAt(2)=='1') ){
                change[0]=String.valueOf(10*(i+1)+(j-1));
                change[1]=board[i+1][j-1];
                change[3]=change[3].concat("h");
            }
